...
function repeatString(str, times) {
  return Number.isInteger(times) && times > 0 ? str.repeat(times) : '';
}
...
function removeFirstOccurrences(str, value) {
  ...
  if (n === -1) return str;
  return str.slice(0, n) + str.slice(n + value.length);
}
...
function sumOfCodes(str = '') {
  ...
  for (let i = 0; i < str.length; i += 1) res += str.charCodeAt(i);
  return res;
}
...
function isPalindrome(str) {
  ...
  while (l < r) {
    if (!s.charAt(l).match(/[a-z]/i)) l += 1;
    else if (!s.charAt(r).match(/[a-z]/i)) r -= 1;
    else {
...
function findLongestWord(sentence) {
  let res = '';
  sentence.split(' ').forEach((i) => {
    res = res.length < i.length ? i : res;
  });
...
function encodeToRot13(str) {
  ...
  const alf13 = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  return str.replace(/[a-z]/gi, (c) => alf13[alf.indexOf(c)]);
}
...
function getCardId(value) {
  const ranks = '♣♦♥♠';
  const st = ranks.indexOf(value.slice(-1)) * 13;
  ...
  return Number.parseInt(v, 10) - 1 + st;
}
